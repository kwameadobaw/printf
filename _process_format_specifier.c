		...
		int *printed_chars)
{
	...
	switch (**format)
	{
		case 'c': buffer[chars_written++] = va_arg(args, int);
			 (*printed_chars)++;
			  break;
		case 's': chars_written += _print_string(&buffer[chars_written],
					  remaining_size - chars_written,
					  va_arg(args, const char *));
			  (*printed_chars) += chars_written;
			  break;
		case '%': buffer[chars_written++] = '%';
			  (*printed_chars)++;
			  break;
		case 'd':
		case 'i': chars_written += _print_number(va_arg(args, int),
					  &buffer[chars_written],
					  remaining_size - chars_written);
			  (*printed_chars) += chars_written;
			  break;
		case 'b': chars_written += _print_binary(va_arg(args, unsigned int),
					  &buffer[chars_written],
					  remaining_size - chars_written);
			  (*printed_chars) += chars_written;
			  break;
